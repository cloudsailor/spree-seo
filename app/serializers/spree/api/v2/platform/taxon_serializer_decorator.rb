        ...
        module TaxonSerializerDecorator
          def self.prepended(base)
            ...
            base.attribute(:combinations_html) do |taxon|
              ...
              body = "<tbody>#{taxon.filter_combinations.map { |f| "<td>#{f.canonical_url}<td><td>#{f.filters.map { |k, v| "#{k}=#{v}"}.join(',')}<td><td>#{f.page_title}</td><td>#{f.meta_description}</td><td class='actions'><span class='d-flex justify-content-end'>Edit Delete</span></td>"}.join('')}</tbody>"

              "<div class='table-responsive border rounded bg-white'><table class='table'>#{headings}#{body}</table></div>"
            end
          end
        end
      end
