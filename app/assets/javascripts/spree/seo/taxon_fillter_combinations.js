document.addEventListener("spree:load", function() {
  window.productTemplate = Handlebars.compile($('#filter_combination_template').text())
  var taxonProducts = $('#taxon_filter_combinations')
  var taxonId = $('#taxonn_id')

  var el = document.getElementById('taxon_filter_combinations')
  if (el) {
    Sortable.create(el, {
      handle: '.sort-handle',
      ...
      forceFallback: true,
      onEnd: function (evt) {
        handlClassificationReposition(evt)
      }
    })
  }

  function handlClassificationReposition(evt) {
